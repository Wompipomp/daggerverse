...
import (
	"context"
	"dagger/security-scan/internal/dagger"
	"encoding/json"
	"fmt"
	"os"
)
...
func (r SecurityScan) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *SecurityScan) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "SecurityScan":
		...
		case "FileSystem":
			...
			return (*SecurityScan).FileSystem(&parent, ctx, dir, severity, exitCode, format)
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("A generated module for SecurityScan functions\n\nThis module has been generated via dagger init and serves as a reference to\nbasic module structure as you get started with Dagger.\n\nTwo functions have been pre-created. You can modify, delete, or add to them,\nas needed. They demonstrate usage of arguments and return types using simple\necho and grep commands. The functions can be called from the dagger CLI or\nfrom one of the SDKs.\n\nThe first line in this comment block is a short description line and the\nrest is a long description with more detail on the module's purpose or usage,\nif appropriate. All modules should have a short description.\n").
			WithObject(
				dag.TypeDef().WithObject("SecurityScan").
					WithFunction(
						dag.Function("FileSystem",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Returns a container that echoes whatever string argument is provided").
							WithArg("dir", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "Directory to scan"}).
							WithArg("severity", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL\"")}).
							WithArg("exitCode", dag.TypeDef().WithKind(IntegerKind).WithOptional(true)).
							WithArg("format", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"table\"")}))), nil
	default:
